...
const ShareSelection = {
  ...
  buildShareUrl() {
    ...
    const mapHook = document.getElementById("tracking-map-frame")?.__trackingMapHook
...
const TrackingMap = {
  ...
  sync(payload) {
    ...
    if (!restoredSharedView && payload.selected_track_id && payload.selected_track_id !== this.lastSelectedTrackId && routeFeatures.length > 0) {
    ...
    } else if (!restoredSharedView && payload.selected_track_id && payload.selected_track_id !== this.lastSelectedTrackId) {
  ...
  restoreSharedView(sharedView) {
    if (!sharedView?.key || sharedView.key === this.lastSharedViewKey) return false
...
const liveSocket = new LiveSocket("/live", Socket, {
  ...
  hooks: {...colocatedHooks, Flash, PreserveScroll, ProfileMenu, ShareSelection, TrackingMap},
})
