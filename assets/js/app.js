...
const PreserveScroll = {
  ...
  beforeUpdate() {
    ...
    this.containerScroll = new Map(
      [...this.el.querySelectorAll("[data-preserve-scroll][id]")].map(
        (node) => [node.id, { left: node.scrollLeft, top: node.scrollTop }],
      ),
    );
    this.detailsState = new Map(
      [...this.el.querySelectorAll("details[data-preserve-open][id]")].map(
        (node) => [node.id, node.open],
      ),
    );
  },
...
const ShareSelection = {
  mounted() {
    this.handleClick = async (event) => {
      ...
      if (!url) {
        this.pushEvent("share_link_failed", {});
        return;
      }
      ...
      try {
        ...
        if (window.prompt) {
          ...
          this.pushEvent("share_link_ready", {});
        } else {
          this.pushEvent("share_link_failed", {});
        }
      }
  ...
  buildShareUrl() {
    const vehicle = this.el.dataset.shareVehicle;
    const identity = this.el.dataset.shareIdentity;
    const range = this.el.dataset.shareRange;

    if (!vehicle || !identity || !range) return null;
    ...
    const mapHook =
      document.getElementById("tracking-map-frame")?.__trackingMapHook;
    const view = mapHook?.currentView?.();

    if (view) {
      url.searchParams.set("lat", view.latitude.toFixed(5));
      url.searchParams.set("lon", view.longitude.toFixed(5));
...
const TrackingMap = {
  mounted() {
    ...
    this.el.__trackingMapHook = this;
    this.syncLegendState();
    ...
    this.map.addControl(
      new window.maplibregl.ScaleControl({ unit: "metric" }),
      "bottom-right",
    );
  ...
  destroyed() {
    if (this.popupRepositionFrame)
      window.cancelAnimationFrame(this.popupRepositionFrame);
    this.el.removeEventListener("click", this.handleLegendClick);
    delete this.el.__trackingMapHook;
  ...
  sync(payload) {
        ...
        ? [
            {
              ...
              geometry: {
                type: "LineString",
                coordinates: payload.route.map((point) => [
                  point.longitude,
                  point.latitude,
                ]),
              },
    ...
    const restoredSharedView = this.restoreSharedView(payload.shared_view);

    if (
      !restoredSharedView &&
      payload.selected_track_id &&
      payload.selected_track_id !== this.lastSelectedTrackId &&
      routeFeatures.length > 0
    ) {
      const bounds = new window.maplibregl.LngLatBounds();
      routeFeatures[0].geometry.coordinates.forEach((coord) =>
        bounds.extend(coord),
      );
      ...
      const selectedTrack = payload.tracks.find(
        (track) => track.id === payload.selected_track_id,
      );

      if (
        selectedTrack &&
        typeof selectedTrack.latitude === "number" &&
        typeof selectedTrack.longitude === "number"
      ) {
        this.map.flyTo({
  ...
  restoreSharedView(sharedView) {
    if (!sharedView?.key || sharedView.key === this.lastSharedViewKey)
      return false;

    this.lastSharedViewKey = sharedView.key;
    this.map.easeTo({
      center: [sharedView.longitude, sharedView.latitude],
  ...
  visibleTrackFilter() {
    ...
    if (filters.length === 0)
      return ["==", ["get", "vehicle_type"], "__hidden__"];
  ...
  applyTrackVisibility() {
    ...
    if (this.map.getLayer("tracks-layer"))
      this.map.setFilter("tracks-layer", trackFilter);
    if (this.map.getLayer("tracks-labels"))
      this.map.setFilter("tracks-labels", trackFilter);
    if (this.map.getLayer("tracks-selected-halo"))
      this.map.setFilter("tracks-selected-halo", haloFilter);

    const routeVisibility = this.selectedTrackVisible() ? "visible" : "none";

    if (this.map.getLayer("route-layer"))
      this.map.setLayoutProperty("route-layer", "visibility", routeVisibility);
  ...
  refreshPopupVisibility() {
    ...
    this.refreshPopup(
      this.lastTrackFeatures,
      this.pendingPayload?.selected_track_id,
    );
  },
  ...
  repositionPopup() {
    if (!this.popup?.isOpen?.() || !this.activePopupCoordinates || !this.map)
      return;
    ...
    const maxWidth = Math.max(
      12 * 16,
      Math.min(20 * 16, containerRect.width - this.popupMargin * 2),
    );
    const maxHeight = Math.max(
      10 * 16,
      containerRect.height - this.popupMargin * 2,
    );

    this.popup.setMaxWidth(`${Math.round(maxWidth)}px`);
    popupElement.style.setProperty(
      "--track-popup-max-height",
      `${Math.round(maxHeight)}px`,
    );

    const point = this.map.project(this.activePopupCoordinates);
    const anchors = this.popupAnchorOrder(
      point,
  ...
  refreshPopup(trackFeatures, selectedTrackId) {
    if (
      ...
      const selectedFeature = trackFeatures.find(
        (track) => track.properties.id === selectedTrackId,
      );
    ...
    const targetId = this.popupPinned
      ? this.activePopupTrackId
      : this.hoveredTrackId;
    ...
    const feature = trackFeatures.find(
      (track) => track.properties.id === targetId,
    );
    ...
    this.openPopup(
      feature,
      this.popupPinned && feature.properties.id === targetId,
    );
  },
  ...
  popupMarkup(properties) {
    const metrics = [
      ...
      {
        label: properties.vehicle_type === "plane" ? "Altitude" : "Status",
        value:
          properties.vehicle_type === "plane"
            ? properties.altitude_label
            : properties.status || "-",
      },
    ];
    ...
    const meta = [
      properties.destination
        ? { label: "Destination", value: properties.destination }
        : null,
      properties.callsign
        ? { label: "Callsign", value: properties.callsign }
        : null,
      properties.registration
        ? {
            label: properties.vehicle_type === "plane" ? "Registration" : "IMO",
            value: properties.registration,
          }
        : null,
      properties.country_display
        ? { label: "Flag", value: properties.country_display }
        : null,
      properties.source_name
        ? { label: "Feed", value: properties.source_name }
        : null,
  ...
  formatAge(value) {
    ...
    const seconds = Math.max(
      0,
      Math.round((Date.now() - observedAt.getTime()) / 1000),
    );
